    ...
    <div className="flex flex-col min-h-screen cp-page">
      ...
      <main className="flex-grow py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-3xl sm:text-4xl font-extrabold cp-text-primary">
            About{' '}
            <span className="bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">
              CareerPilot AI
            </span>
          </h1>
          <p className="mt-4 text-lg cp-text-secondary max-w-2xl mx-auto">
        ...
        <div className="mt-16 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {sections.map((section, idx) => (
            <div key={idx} className="cp-card p-8">
              <div className="p-3 bg-indigo-500/10 rounded-xl border border-indigo-500/20 text-indigo-400 inline-block">
                <section.icon className="h-6 w-6" />
              </div>
              <h3 className="text-lg font-bold cp-text-primary mt-6">{section.title}</h3>
              <p className="cp-text-secondary text-sm mt-3 leading-relaxed">{section.content}</p>
            </div>
          ))}
        </div>

        <div className="mt-16 text-center max-w-xl mx-auto border-t border-cp-border pt-10">
          <h2 className="text-xl font-bold cp-text-primary">System Platform Architecture</h2>
          <p className="cp-text-secondary text-sm mt-2">
            CareerPilot AI operates on an asynchronous distributed event model, parsing resume metrics,
            parsing public databases, and storing structured logs securely.
          </p>
        </div>
      </main>
